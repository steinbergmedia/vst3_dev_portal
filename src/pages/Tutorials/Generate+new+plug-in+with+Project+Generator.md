...
We need to set our internal **mGain** with its required value from the host. This is the first step of the process method. Parse the parameter change coming from the host in the structure *data.inputParameterChanges* for the current audio block to process. Be sure to add `#include "public.sdk/source/vst/vstaudioprocessoralgo.h` at top of the file plugprocessor.cpp! This includes some helpers to access audio buffer.
